...
public class Producto implements Comparable<Producto> {
	...
	/**
	 * Constructor que recibe y almacena la informaci�n de un prodcuto, le asigna
	 * como fecha de �ltima revisi�n el momento en que se llama a este constructor.
	 * 
	...
	/**
	 * Constructor temporal, que se eliminar�, para poder probar el tema de
	 * colecciones antes de hacer el web scrapping de los supermercados
	 * 
	 * @deprecated
	...
	/**
	 * obtiene la url desde la que se hace web scrapping a las dem�s propiedades del
	 * producto
	 * 
	...
	/**
	 * Cambia la url para hacer web scrapping al producto, y recarga toda su
	 * informaci�n.
	 * 
	...
	/**
	 * Funci�n que a partir de una URL obtiene mediante web scrapping el nombre,
	 * supermercado y precio del producto.
	 * 
	 * @throws IOException
	 */
	public void recargarInfo() throws IOException {
		...
		if (dirUrl.getAuthority().contains("dia")) {
			...
			if (texto.contains("big-price")) {
				posicionPrecio = texto.indexOf("<span class=\"big-price\" itemprop=\"price\">");
				texto = texto.substring(posicionPrecio + 41);
				
			} else {
				System.err.println("El precio no se puede encontrar");
			}
			...
			this.supermercado = "Dia";
			this.ultimaRevision = new java.sql.Timestamp(Calendar.getInstance().getTime().getTime());

			int posicionNombre = textoParaNombre.indexOf("<h1 itemprop=\"name\">");
			textoParaNombre = textoParaNombre.substring(posicionNombre + 20);
